import postmarkClient from "../middleswares/postmark-client.js";
import { generateActivationToken } from "../utils/tokenUtils.js";

// Function to send the welcome email
export const postmarkWelcomeEmail = async (lead) => {
    try {
        ...
        // 1. The HTML body remains identical (it's perfect as is)
        const htmlBody = `
            <!DOCTYPE html
...
<div style="height:20px"></div>
        ...
        // 2. CONSTRUCT THE POSTMARK MESSAGE OBJECT
        const postmarkOptions = {
            // Postmark requires 'From' and 'To' to be capitalized
            From: process.env.EMAIL_SENDER_ADDRESS, // 👈 Use the address linked to your Postmark Server
            To: email,
            Subject: "Welcome to Red2Roast!",
            HtmlBody: htmlBody, // Postmark uses HtmlBody instead of html

            // Optional: Highly recommended for deliverability and analytics in Postmark
            MessageStream: "outbound"
        };

        // 3. SEND THE EMAIL using the Postmark client
        const response = await postmarkClient.sendEmail(postmarkOptions);

        // 4. Log the success using Postmark's response data
        console.log(`Welcome email sent via Postmark. MessageID: ${response.MessageID}`);
    } catch (error) {
        // Postmark errors are usually more descriptive
        console.error("Error sending email notification via Postmark:", error.message || error);
    }
};